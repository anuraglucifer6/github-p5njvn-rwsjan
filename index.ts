...
function initMap(): void {
  ...
  stationData.forEach(({
    ...
    if (freq > 0) {
      ...
      const infoString = `<h3>${code}</h3>${name}<br>${cityName}, ${stateName}<br>Boading : ${boardingFreq}<br>Deboarding : ${departureFreq}<br>Halting : ${asHalt} trains<br>Non Haltiing : ${asNonHalt || 0} trains`;
      addMarker(
        location,
        {
